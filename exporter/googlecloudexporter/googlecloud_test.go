...
func TestGoogleCloudTraceExport(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			componentSettings := component.ComponentSettings{Logger: zap.NewNop(), ApplicationStartInfo: component.ApplicationStartInfo{Version: "v0.0.1"}}
			sde, err := newGoogleCloudTracesExporter(test.cfg, componentSettings)
			if test.expectedErr != "" {
