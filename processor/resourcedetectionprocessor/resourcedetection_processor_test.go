	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			factory.resourceProviderFactory = internal.NewProviderFactory(
				map[internal.DetectorType]internal.DetectorFactory{"mock": func(component.ComponentSettings, internal.DetectorConfig) (internal.Detector, error) {
					return md1, nil
				}})
			...
			rtp, err := factory.createTracesProcessor(context.Background(), component.ComponentSettings{Logger: zap.NewNop()}, cfg, ttn)
			...
			rmp, err := factory.createMetricsProcessor(context.Background(), component.ComponentSettings{Logger: zap.NewNop()}, cfg, tmn)
			...
			rlp, err := factory.createLogsProcessor(context.Background(), component.ComponentSettings{Logger: zap.NewNop()}, cfg, tln)
...
func benchmarkConsumeTraces(b *testing.B, cfg *Config) {
	...
	processor, _ := factory.CreateTracesProcessor(context.Background(), component.ComponentSettings{Logger: zap.NewNop()}, cfg, sink)
...
func benchmarkConsumeMetrics(b *testing.B, cfg *Config) {
	...
	processor, _ := factory.CreateMetricsProcessor(context.Background(), component.ComponentSettings{Logger: zap.NewNop()}, cfg, sink)
...
func benchmarkConsumeLogs(b *testing.B, cfg *Config) {
	...
	processor, _ := factory.CreateLogsProcessor(context.Background(), component.ComponentSettings{Logger: zap.NewNop()}, cfg, sink)
